...
export default {
            ...
            if(typeof field === "function") {
                ...
                if(typeof field === "object") {
                    if(field.wrap) {
                        ...
                        let rel = await models[field.table].findOne({attributes: ["id"], order: models.Sequelize.literal(config.database.dialect === 'postgres' ? 'random()' : 'rand()')});
                        if(rel)
                            data[key] = rel.id;
                    }
                }
    ...
    extractModelAttrib: function(modelName) {
        ...
        if(model) {
            Object.keys(model.rawAttributes).forEach((column) => {
                let info = model.rawAttributes[column];
                if(info.primaryKey) {
